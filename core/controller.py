...
from core.command import DriveCommand
...
class SwitchController(Controller):
    ...
    def read(self) -> ControllerState:
        ...
        try:
            ...
            state.command.throttle = 0.0 if abs(raw_t) < self._config.switch.switch_deadzone else raw_t
            state.command.steering = 0.0 if abs(raw_s) < self._config.switch.switch_deadzone else raw_s
